...
export class CandidateService {
  ...
  async GetListPagination(
    queryPagination: CandidateQueryDto,
    ...
    try {
      ...
      const experienceQuery = queryPagination.experience
        ? { experience: { $lte: queryPagination.experience } }
        : {};
      const searchQuery = queryPagination.search
        ? { name: { $regex: queryPagination.search, $options: 'i' } }
        : {};
      const locationQuery = queryPagination.location
        ? { location: { $regex: queryPagination.location, $options: 'i' } }
        : {};
      const industryQuery = queryPagination.industry
        ? { industry: { $regex: queryPagination.industry, $options: 'i' } }
        : {};
      const genderQuery = queryPagination.gender
        ? { gender: { $regex: queryPagination.gender, $options: 'i' } }
        : {};

      let educationQuery = {};
      if (queryPagination.education === 'university') {
        educationQuery = {
          educationLevel: { $regex: 'Đại học', $options: 'i' },
        };
      } else if (queryPagination.education === 'college') {
        educationQuery = {
          educationLevel: { $regex: 'Cao đẳng', $options: 'i' },
        };
  ...
  async GetListIndustryOfCandidate() {
    try {
      ...
      console.error(
        'Lỗi lấy danh sách danh mục của các ứng viên:',
        error.message,
      );
      throw new InternalServerErrorException(
  ...
  async getCandidateById(id: string): Promise<Candidate> {
    try {
      ...
      handleServiceError(error, 'CandidateService.getCandidateById');
    }
  }
  ...
  async getAvatarOfCandidate(id: string) {
    try {
      const logo = await this.candidateModel
        .findById(id)
        .select('avatar')
        .exec();
      return logo;
  ...
  async getIndustryOfCandidates() {
    try {
      ...
      console.error(
        'Lỗi lấy danh mục nghề nghiệp của các ứng viên:',
        error.message,
      );
      throw new InternalServerErrorException(
