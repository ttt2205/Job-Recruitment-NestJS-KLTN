...
export class CompanyService {
  ...
  async createNewCompany(data: CreateCompanyDto) {
    try {
  ...
  async GetListPagination(queryPagination: CompanyQueryDto) {
    try {
      ...
      const foundationDateQuery =
        foundationDateMin && foundationDateMax
          ? {
      ...
      const companiesWithJobCount = await Promise.all(
        data.map(async (company) => {
          const jobCount = await this.jobService.countJobsByCompanyId(
  ...
  async getIndustryOfCompanies() {
    try {
      const industries = await this.companyModel
        .distinct('primaryIndustry')
        .exec();
      return industries;
    } catch (error) {
